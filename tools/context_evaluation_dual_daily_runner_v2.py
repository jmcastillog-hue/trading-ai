...
CAPABILITY = "CONTEXT_EVALUATION_DUAL_DAILY_RUNNER_V2"
COHORT_ID = "CONTEXT_LEVEL_A_DUAL_DAILY_30_V2"
PLAN_RELATIVE = Path(
...
def run_name(slot_id: str, anchor: datetime) -> str:
    return f"D30V2_{slot_id}_{stamp_for_anchor(anchor)}"
...
def self_check(repo: Path) -> int:
    print("=== DUAL DAILY RUNNER SELF CHECK V2 ===")
    ...
    context_namespace = namespace_from_source(
        ...
        "dual_daily_context_path_rebind_self_check",
    )
    ...
    outcome_namespace = namespace_from_source(
        ...
        "dual_daily_outcome_path_rebind_self_check",
    )
...
def initialize_root(repo: Path) -> int:
    print("=== INITIALIZE DUAL DAILY COHORT ROOT V2 ===")
...
def status(repo: Path, slot_id: str | None) -> int:
    print("=== DUAL DAILY COHORT STATUS V2 ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Supervised, plan-bound runner for CONTEXT_LEVEL_A_DUAL_DAILY_30_V2"
        )
    )

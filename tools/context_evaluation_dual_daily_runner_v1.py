...
CAPABILITY = "CONTEXT_EVALUATION_DUAL_DAILY_RUNNER_V1"
COHORT_ID = "CONTEXT_LEVEL_A_DUAL_DAILY_30_V1"
SUPERSEDED_BY = "CONTEXT_LEVEL_A_DUAL_DAILY_30_V2"
PLAN_RELATIVE = Path(
...
def run_name(slot_id: str, anchor: datetime) -> str:
    return f"D30_{slot_id}_{stamp_for_anchor(anchor)}"
...
def validate_plan_with_frozen_contract(repo: Path) -> dict[str, Any]:
    ...
    if min(spacings) < timedelta(hours=4):
        fail("DUAL_DAILY_MINIMUM_SPACING_INVALID")
    if min(spacings) != timedelta(hours=11):
        fail("DUAL_DAILY_DST_MINIMUM_SPACING_UNEXPECTED")
    if max(spacings) != timedelta(hours=12):
        fail("DUAL_DAILY_MAXIMUM_SPACING_UNEXPECTED")
    print("FROZEN_PLAN_CONTRACT_VALIDATED=True")
    print("PLANNED_SLOT_COUNT=30")
    print("MINIMUM_ACTUAL_SLOT_SPACING_HOURS=11")
    print("MAXIMUM_ACTUAL_SLOT_SPACING_HOURS=12")
...
def transform_context_template(
    ...
    *,
    ...
    compile(source, f"<dual_daily_context_{slot_id}>", "exec")
    return source
...
def transform_outcome_template(
    ...
    *,
    ...
    compile(source, f"<dual_daily_outcome_{slot_id}>", "exec")
    return source
...
def self_check(repo: Path) -> int:
    print("=== DUAL DAILY RUNNER SELF CHECK V1 ===")
...
def initialize_root(repo: Path) -> int:
    fail(
        "COHORT_SUPERSEDED_FAIL_CLOSED "
        f"cohort={COHORT_ID} superseded_by={SUPERSEDED_BY}"
    )
    print("=== INITIALIZE DUAL DAILY COHORT ROOT V1 ===")
    head = verify_real_repo(repo)
    validate_plan_with_frozen_contract(repo)
    sys.path.insert(0, str(repo))
    from src.evaluation import context_evaluation_prospective_cohort_v1 as cohort
    ...
    print("OFFICIAL_APPEND_EXECUTED=False")
    print("DUAL_DAILY_COHORT_ROOT_INITIALIZED=True")
    return 0
...
def status(repo: Path, slot_id: str | None) -> int:
    print("=== DUAL DAILY COHORT STATUS V1 ===")
...
def offline_validate_context(repo: Path, slot_id: str) -> int:
    ...
    namespace = namespace_from_source(source, f"dual_daily_context_{slot_id}")
...
def offline_validate_binding(repo: Path, slot_id: str) -> int:
    ...
    namespace = namespace_from_source(source, f"dual_daily_outcome_{slot_id}")
    return int(namespace["offline_validate"](repo))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Supervised, plan-bound runner for CONTEXT_LEVEL_A_DUAL_DAILY_30_V1"
        )
    )
    ...
    if args.initialize_root or args.execute_context or args.bind_outcomes:
        fail(
            "COHORT_SUPERSEDED_FAIL_CLOSED "
            f"cohort={COHORT_ID} superseded_by={SUPERSEDED_BY}"
        )
...
if __name__ == "__main__":
    try:
        ...
        print("DUAL_DAILY_RUNNER_STATUS=FAILED", file=sys.stderr)

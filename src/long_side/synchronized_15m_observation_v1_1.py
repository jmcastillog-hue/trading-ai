...
IMPLEMENTATION_SCHEMA_VERSION = "SYNCHRONIZED_15M_OBSERVATION_IMPLEMENTATION_V1_1"
SESSION_SCHEMA_VERSION = "SYNCHRONIZED_15M_OBSERVATION_SESSION_V1_1"
...
IMPLEMENTATION_OR_REPAIR_ATTEMPT = 3
...
TEMPORAL_ALIGNMENT_POLICY_VERSION = "SYNCHRONIZED_COMPONENT_TIMESTAMP_ELIGIBILITY_V1"
HISTORICAL_COMPONENT_NAMES = (
    "open_interest_history",
    "taker_buy_sell_volume",
    "global_long_short_account_ratio",
)
POINT_IN_TIME_COMPONENT_NAMES = (
    "order_book",
...
class SynchronizedObservationV11Error(RuntimeError):
...
def _req(ok: bool, code: str, message: str) -> None:
    if not ok:
        raise SynchronizedObservationV11Error(code, message)
...
def _parse_utc(value: Any, field: str) -> datetime:
    ...
    try:
        ...
        raise SynchronizedObservationV11Error("UTC_TIMESTAMP_INVALID", field) from exc
...
def _read_primary_evaluation(package_directory: Path) -> dict[str, bool]:
    ...
    _req(
        checks_path.is_file() and not checks_path.is_symlink(),
        "PACKAGE_CHECKS_MISSING",
        str(checks_path),
    )
    ...
    for field in fields:
        _req(
            field in checks and isinstance(checks[field], bool),
...
def _micro_capture_safe(result: Mapping[str, Any], expected_output: Path) -> None:
    _req(
        str(Path(str(result["output_directory"])).resolve()) == str(expected_output.resolve()),
        "MICROSTRUCTURE_OUTPUT_MISMATCH",
        "output",
    )
    _req(
        int(result["request_count"]) == EXPECTED_MICROSTRUCTURE_REQUESTS_PER_CYCLE,
        "MICROSTRUCTURE_REQUEST_COUNT_INVALID",
        "request_count",
    )
    _req(
        int(result["depth_limit_requested"]) == MICROSTRUCTURE_DEPTH_LIMIT,
        "MICROSTRUCTURE_DEPTH_LIMIT_INVALID",
        "depth_limit",
    )
    _req(
        tuple(int(x) for x in result["depth_bands_bps"]) == DEPTH_BANDS_BPS,
        "MICROSTRUCTURE_DEPTH_BANDS_INVALID",
        "bands",
    )
    _req(
        bool(result["foreground_only"]) and bool(result["public_read_only"]),
        "MICROSTRUCTURE_MODE_INVALID",
        "mode",
    )
...
def _read_microstructure_summary(
    ...
    _req(
        int(validation["request_count"]) == EXPECTED_MICROSTRUCTURE_REQUESTS_PER_CYCLE,
        "MICROSTRUCTURE_VALIDATION_INVALID",
        "request_count",
    )
    _req(
        int(validation["depth_limit_requested"]) == MICROSTRUCTURE_DEPTH_LIMIT,
        "MICROSTRUCTURE_VALIDATION_INVALID",
        "depth_limit",
    )
    _req(
        tuple(int(x) for x in validation["depth_bands_bps"]) == DEPTH_BANDS_BPS,
        "MICROSTRUCTURE_VALIDATION_INVALID",
        "bands",
    )
    ...
    _req(
        ...
        "MICROSTRUCTURE_SUMMARY_MISSING",
        str(summary_path),
    )
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    _req(
        summary["capability"] == "PUBLIC_READ_ONLY_MICROSTRUCTURE_SNAPSHOT_V1_1",
        "MICROSTRUCTURE_CAPABILITY_INVALID",
        "capability",
    )
    _req(
        summary["provider"] == "BINANCE_USDM_PUBLIC_REST",
        "MICROSTRUCTURE_PROVIDER_INVALID",
        "provider",
    )
    _req(
        summary["symbol"] == "BTCUSDT" and summary["timeframe"] == "15m",
        "MICROSTRUCTURE_IDENTITY_INVALID",
        "identity",
    )
    _req(
        int(summary["request_count"]) == EXPECTED_MICROSTRUCTURE_REQUESTS_PER_CYCLE,
        "MICROSTRUCTURE_REQUEST_COUNT_INVALID",
        "summary",
    )
    _req(
        int(summary["depth_limit_requested"]) == MICROSTRUCTURE_DEPTH_LIMIT,
        "MICROSTRUCTURE_DEPTH_LIMIT_INVALID",
        "summary",
    )
    _req(
        tuple(int(x) for x in summary["depth_bands_bps"]) == DEPTH_BANDS_BPS,
        "MICROSTRUCTURE_DEPTH_BANDS_INVALID",
        "summary",
    )
    for field in MICROSTRUCTURE_FALSE_FIELDS:
...
def _depth_context(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    minimum_usable = all(
        bands[str(bps)]["usable_for_context"]
        for bps in REQUIRED_DEPTH_BANDS_BPS
    )
    return {
...
def _historical_component_eligibility(
    *,
    component_name: str,
    component_timestamp_utc: Any,
    reference_boundary: datetime,
) -> dict[str, Any]:
    component_time = _parse_utc(
        component_timestamp_utc,
        f"{component_name}.timestamp_utc",
    )
    delta_seconds = (component_time - reference_boundary).total_seconds()
    equal = component_time == reference_boundary
    return {
        "component_name": component_name,
        "temporal_semantics": "PROVIDER_TIMESTAMPED_HISTORICAL_SERIES",
        "component_timestamp_utc": _utc(component_time),
        "reference_boundary_utc": _utc(reference_boundary),
        "timestamp_delta_seconds": delta_seconds,
        "timestamp_equal_reference_boundary": equal,
        "historical_interval_equivalence_claimed": False,
        "usable_for_synchronized_context": equal,
        "misalignment_reason": None if equal else "PROVIDER_TIMESTAMP_NOT_EQUAL_REFERENCE_BOUNDARY",
    }


def _point_in_time_component(
    *,
    component_name: str,
    reference_boundary: datetime,
    component_timestamp_utc: Any | None,
) -> dict[str, Any]:
    component_time: datetime | None = None
    delta_seconds: float | None = None
    observed_at_or_after_reference: bool | None = None
    if component_timestamp_utc is not None:
        component_time = _parse_utc(
            component_timestamp_utc,
            f"{component_name}.timestamp_utc",
        )
        delta_seconds = (component_time - reference_boundary).total_seconds()
        observed_at_or_after_reference = component_time >= reference_boundary
    return {
        "component_name": component_name,
        "temporal_semantics": "POINT_IN_TIME_AFTER_REFERENCE",
        "component_timestamp_utc": _utc(component_time) if component_time else None,
        "reference_boundary_utc": _utc(reference_boundary),
        "timestamp_delta_seconds": delta_seconds,
        "observed_at_or_after_reference": observed_at_or_after_reference,
...
def assess_component_temporal_eligibility(
    summary: Mapping[str, Any],
) -> dict[str, Any]:
    synchronization = summary["synchronization"]
    reference_boundary = _parse_utc(
        synchronization["reference_boundary_utc"],
        "synchronization.reference_boundary_utc",
    )
    ...
    _req(
        reference_boundary - reference_close == timedelta(milliseconds=1),
        "MICROSTRUCTURE_REFERENCE_BOUNDARY_INVALID",
        "reference boundary must be exactly 1ms after candle close",
    )

    historical = {
        "open_interest_history": _historical_component_eligibility(
            component_name="open_interest_history",
            component_timestamp_utc=summary["open_interest"]["latest_15m"]["timestamp_utc"],
            reference_boundary=reference_boundary,
        ),
        "taker_buy_sell_volume": _historical_component_eligibility(
            component_name="taker_buy_sell_volume",
            component_timestamp_utc=summary["taker_buy_sell_volume"]["latest_15m"]["timestamp_utc"],
            reference_boundary=reference_boundary,
        ),
        "global_long_short_account_ratio": _historical_component_eligibility(
            component_name="global_long_short_account_ratio",
            component_timestamp_utc=summary["global_long_short_account_ratio"]["latest_15m"]["timestamp_utc"],
            reference_boundary=reference_boundary,
        ),
    }
    ...
    aligned = [
        ...
        if historical[name]["usable_for_synchronized_context"]
    ]
    misaligned = [
        name
        for name in HISTORICAL_COMPONENT_NAMES
        if not historical[name]["usable_for_synchronized_context"]
    ]

    point_in_time = {
        "order_book": _point_in_time_component(
            component_name="order_book",
            reference_boundary=reference_boundary,
            component_timestamp_utc=None,
        ),
        "current_open_interest": _point_in_time_component(
            component_name="current_open_interest",
            reference_boundary=reference_boundary,
            component_timestamp_utc=summary["open_interest"].get("current_time_utc"),
        ),
        "mark_price_funding": _point_in_time_component(
            component_name="mark_price_funding",
            reference_boundary=reference_boundary,
            component_timestamp_utc=summary["mark_price_funding"].get("provider_time_utc"),
        ),
    }

    upstream = list(synchronization.get("aligned_15m_components", []))
    return {
        "policy_version": TEMPORAL_ALIGNMENT_POLICY_VERSION,
        "reference_closed_candle_utc": _utc(reference_close),
        "reference_boundary_utc": _utc(reference_boundary),
        "historical_component_timestamp_alignment_required_for_use": True,
        "exact_reference_boundary_equality_required": True,
        "historical_interval_equivalence_claimed": False,
        "upstream_reported_aligned_15m_components": upstream,
        "historical_components": historical,
        "aligned_historical_components": aligned,
        "misaligned_historical_components": misaligned,
        "all_historical_components_timestamp_aligned": len(misaligned) == 0,
        "point_in_time_components": point_in_time,
        "point_in_time_components_are_not_historical_reconstruction": True,
        "misaligned_historical_component_preserved_but_not_usable": True,
    }


def build_microstructure_context_v1_1(
    summary: Mapping[str, Any],
) -> dict[str, Any]:
    depth = _depth_context(summary)
    eligibility = assess_component_temporal_eligibility(summary)

    open_interest = deepcopy(summary["open_interest"])
    taker = deepcopy(summary["taker_buy_sell_volume"])
    global_ratio = deepcopy(summary["global_long_short_account_ratio"])
    mark = deepcopy(summary["mark_price_funding"])
    interpretation = deepcopy(summary["interpretation_constraints"])
    synchronization = deepcopy(summary["synchronization"])

    mapping = {
        "open_interest_history": open_interest,
        "taker_buy_sell_volume": taker,
        "global_long_short_account_ratio": global_ratio,
    }
    for name, payload in mapping.items():
        temporal = deepcopy(eligibility["historical_components"][name])
        payload["latest_15m_temporal_eligibility"] = temporal
        payload["latest_15m_usable_for_synchronized_context"] = bool(
            temporal["usable_for_synchronized_context"]
        )

    synchronization[
        "upstream_reported_aligned_15m_components"
    ] = eligibility["upstream_reported_aligned_15m_components"]
    synchronization["aligned_15m_components"] = eligibility[
        "aligned_historical_components"
    ]
    synchronization["misaligned_15m_components"] = eligibility[
        "misaligned_historical_components"
    ]
    synchronization["alignment_recomputed_by_synchronized_observation_v1_1"] = True
    synchronization["historical_interval_equivalence_claimed"] = False
    ...
    return {
        ...
        "complete_bands_bps": depth["complete_bands_bps"],
        "incomplete_bands_bps": depth["incomplete_bands_bps"],
        "minimum_depth_context_usable": depth["minimum_depth_context_usable"],
        "incomplete_depth_extrapolation_allowed": False,
        "best_bid": depth["best_bid"],
        "best_ask": depth["best_ask"],
        "mid_price": depth["mid_price"],
        "spread_bps": depth["spread_bps"],
        "furthest_bid_distance_bps": depth["furthest_bid_distance_bps"],
        "furthest_ask_distance_bps": depth["furthest_ask_distance_bps"],
        "bands": depth["bands"],
        ...
        "component_temporal_eligibility": eligibility,
        "interpretation_constraints": interpretation,
...
def validate_synchronized_observation_session_v1_1(
    directory: Path | str,
) -> dict[str, Any]:
    ...
    for line in manifest_lines:
        ...
        names.append(name)
    _req(
        sorted(names) == sorted([EVENTS_FILENAME, SUMMARY_FILENAME]),
    ...
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    _req(
        summary["session_schema_version"] == SESSION_SCHEMA_VERSION,
        "SESSION_SCHEMA_INVALID",
        "schema",
    )
    _req(summary["capability"] == CAPABILITY, "SESSION_CAPABILITY_INVALID", "capability")
    _req(
        int(summary["network_requests_per_completed_cycle"])
        == EXPECTED_NETWORK_REQUESTS_PER_CYCLE,
        "SESSION_REQUEST_CONTRACT_INVALID",
        "per cycle",
    )
    _req(
        int(summary["network_request_count"])
        == int(summary["completed_cycles"]) * EXPECTED_NETWORK_REQUESTS_PER_CYCLE,
        "SESSION_REQUEST_COUNT_INVALID",
        "total",
    )
    _req(
        summary["spot_futures_closed_candle_match_required"] is True,
        "SESSION_SYNCHRONIZATION_CONTRACT_INVALID",
        "required",
    )
    _req(
        summary["microstructure_mutates_frozen_candidate_rule"] is False,
        "SESSION_PRIMARY_RULE_INVALID",
        "microstructure",
    )
    _req(
        summary["temporal_alignment_policy_version"] == TEMPORAL_ALIGNMENT_POLICY_VERSION,
        "SESSION_TEMPORAL_POLICY_INVALID",
        "policy",
    )
    _req(
        summary["historical_component_timestamp_alignment_required_for_use"] is True,
        "SESSION_TEMPORAL_POLICY_INVALID",
    ...
    _req(
        len(cycles) == int(summary["completed_cycles"]),
        "SESSION_EVENT_COUNT_INVALID",
        "cycles",
    )

    misaligned_observations = 0
    for event in cycles:
        _req(
            event["synchronization"]["closed_candle_match"] is True,
            "SESSION_CANDLE_MISMATCH",
            "event",
        )
        spot_time = _parse_utc(
            event["synchronization"]["spot_latest_closed_candle_utc"],
            "event.spot",
        )
        futures_time = _parse_utc(
            event["synchronization"]["futures_reference_closed_candle_utc"],
            "event.futures",
        )
        _req(spot_time == futures_time, "SESSION_CANDLE_MISMATCH", "event time")

        micro = event["microstructure_context"]
        _req(
            micro["microstructure_can_create_candidate"] is False,
            "SESSION_PRIMARY_RULE_INVALID",
            "create candidate",
        )
        _req(
            micro["microstructure_can_cancel_candidate"] is False,
            "SESSION_PRIMARY_RULE_INVALID",
            "cancel candidate",
        )

        eligibility = micro["component_temporal_eligibility"]
        _req(
            eligibility["policy_version"] == TEMPORAL_ALIGNMENT_POLICY_VERSION,
            "SESSION_TEMPORAL_POLICY_INVALID",
            "event policy",
        )
        aligned = []
        misaligned = []
        for name in HISTORICAL_COMPONENT_NAMES:
            item = eligibility["historical_components"][name]
            equal = bool(item["timestamp_equal_reference_boundary"])
            usable = bool(item["usable_for_synchronized_context"])
            _req(
                equal == usable,
                "SESSION_TEMPORAL_ELIGIBILITY_INVALID",
                name,
            )
            payload = (
                micro["open_interest"]
            ...
            _req(
                payload["latest_15m_usable_for_synchronized_context"] == usable,
                "SESSION_TEMPORAL_ELIGIBILITY_INVALID",
                f"{name} payload",
            )
            if usable:
                ...
                misaligned_observations += 1

        _req(
            eligibility["aligned_historical_components"] == aligned,
            "SESSION_TEMPORAL_ALIGNMENT_LIST_INVALID",
            "aligned",
        )
        _req(
            eligibility["misaligned_historical_components"] == misaligned,
            "SESSION_TEMPORAL_ALIGNMENT_LIST_INVALID",
            "misaligned",
        )
        _req(
            micro["synchronization"]["aligned_15m_components"] == aligned,
            "SESSION_TEMPORAL_ALIGNMENT_LIST_INVALID",
            "derived synchronization",
        )
        _req(
            micro["synchronization"]["misaligned_15m_components"] == misaligned,
            "SESSION_TEMPORAL_ALIGNMENT_LIST_INVALID",
            "derived synchronization",
        )

        for band in micro["bands"].values():
            if band["coverage_complete"]:
                _req(
                    band["usable_for_context"] is True
                    and band["notional_imbalance_usable"] is not None,
                    "SESSION_DEPTH_USAGE_INVALID",
                    "complete band",
                )
            else:
                _req(
                    band["usable_for_context"] is False
                    and band["notional_imbalance_usable"] is None,
                    "SESSION_DEPTH_USAGE_INVALID",
                    "incomplete band",
                )
    ...
    return {
        ...
        "misaligned_historical_component_observations": misaligned_observations,
    }


def run_bounded_synchronized_15m_session_v1_1(
    *,
    repo_root: Path | str,
    ...
    _req(
        isinstance(max_cycles, int) and 1 <= max_cycles <= MAX_OBSERVATION_CYCLES,
    ...
    _append(
        events_path,
        {
            ...
            "temporal_alignment_policy_version": TEMPORAL_ALIGNMENT_POLICY_VERSION,
            "historical_component_timestamp_alignment_required_for_use": True,
    ...
    misaligned_historical_component_observations = 0
    ...
    try:
        for index in range(1, max_cycles + 1):
            ...
            latest_dt = _parse_utc(
                spot["latest_closed_candle_utc"],
                "spot.latest_closed_candle_utc",
            )
            _req(
                prior is None or latest_dt > prior,
                "DUPLICATE_OR_OLD_CANDLE",
                "spot candle",
            )
            prior = latest_dt
            ...
            metadata = json.loads(
                Path(str(spot["metadata_json"])).read_text(encoding="utf-8")
            )
            ...
            _req(
                bool(package["candidate_detected"])
                == primary_evaluation["candidate_detected"],
            ...
            micro_summary = _read_microstructure_summary(
                micro_output,
                micro_validate_fn,
            )
            ...
            _req(
                latest_dt == futures_dt,
                "SPOT_FUTURES_CANDLE_MISMATCH",
                f"{latest_dt.isoformat()} != {futures_dt.isoformat()}",
            )

            micro_context = build_microstructure_context_v1_1(micro_summary)
            misaligned_historical_component_observations += len(
                micro_context["component_temporal_eligibility"][
                    "misaligned_historical_components"
                ]
            )
            ...
            _append(
                events_path,
                {
                    ...
                    "microstructure_manifest_sha256": _sha(
                        micro_output / "manifest.sha256"
                    ),
                    ...
                    "eligible_for_real_human_review": bool(
                        package["eligible_for_real_human_review"]
                    ),
    ...
    summary = {
        ...
        "historical_interval_equivalence_claimed": False,
        "misaligned_historical_component_preserved_but_not_usable": True,
        "misaligned_historical_component_observations": (
            misaligned_historical_component_observations
        ),
    ...
    validation = validate_synchronized_observation_session_v1_1(out)
    return {
        "capability": CAPABILITY,

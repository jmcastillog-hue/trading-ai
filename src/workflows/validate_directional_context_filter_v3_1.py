...
from src.market_structure.directional_context_filter_v3_1 import (
    enrich_with_directional_context_v3_1,
)
...
def build_combined_context_dataset_v3_1(
    csv_15m: Path,
    csv_1h: Path,
    csv_4h: Path,
    enriched_csv: Path,
    ...
    directional_csv = reports_dir / f"{base_name}_directional_v3_1_only.csv"
    ...
    directional_df = enrich_with_directional_context_v3_1(
        entry_csv_path=csv_15m,
        h1_csv_path=csv_1h,
        h4_csv_path=csv_4h,
    ...
    for col, value in fill_map.items():
        if col in combined_df.columns:
            combined_df[col] = combined_df[col].fillna(value)
...
def long_v2_with_directional_context_v3_1(df, index: int, config=None) -> str:
    ...
    if bool(row.get("long_allowed_v3_1", False)):
        return "LONG"

    return "NONE"


def short_mtf_with_directional_context_v3_1(df, index: int, config=None) -> str:
...
def short_mtf_liquidity_v2_with_directional_context_v3_1(
    df,
    index: int,
    config=None,
) -> str:
...
def add_trade_context(
    ...
    for _, trade in trades_df.iterrows():
        ...
        row["long_context_decision_v3_1"] = entry_row.get(
            "long_context_decision_v3_1",
            "UNKNOWN",
        )
        row["short_context_decision_v3_1"] = entry_row.get(
            "short_context_decision_v3_1",
            "UNKNOWN",
        )
        row["long_allowed_v3_1"] = entry_row.get("long_allowed_v3_1", False)
        row["short_allowed_v3_1"] = entry_row.get("short_allowed_v3_1", False)
...
def validate_window(
    ...
    data_dir = Path("data") / "directional_context_v3_1"
    reports_dir = Path("reports") / "directional_context_v3_1"
    ...
    enriched_csv = reports_dir / f"{base_name}_directional_context_v3_1.csv"

    print()
    print(f"DIRECTIONAL CONTEXT V3.1 WINDOW: {window_name}")
    print("=" * 100)
    ...
    strategies = [
        ...
        {
            "strategy_name": "LONG_V2_DIRECTIONAL_V3_1",
            "direction": "LONG",
            "strategy_func": long_v2_with_directional_context_v3_1,
            "config": build_long_config(),
        },
        {
        ...
        {
            "strategy_name": "SHORT_FIB_V5_MTF_DIRECTIONAL_V3_1",
            "direction": "SHORT",
        ...
        {
            "strategy_name": "SHORT_FIB_V5_MTF_LIQUIDITY_V2_DIRECTIONAL_V3_1",
            "direction": "SHORT",
...
def print_base_vs_v3_1(results_df: pd.DataFrame):
    print()
    print("BASE VS DIRECTIONAL V3.1 COMPARISON")
    print("=" * 100)

    pairs = [
        ("LONG_V2_BASE", "LONG_V2_DIRECTIONAL_V3_1"),
        ("SHORT_FIB_V5_MTF_BASE", "SHORT_FIB_V5_MTF_DIRECTIONAL_V3_1"),
        (
        ...
        rows.append(
            {
                ...
                "trades_removed": int(
                    base["total_trades"].sum() - filtered["total_trades"].sum()
                ),
                ...
                "filtered_avg_pf": pd.to_numeric(
                    filtered["profit_factor"],
                    errors="coerce",
                ).mean(),
...
def print_v3_1_context_distribution(trades_df: pd.DataFrame):
    print()
    print("DIRECTIONAL CONTEXT V3.1 TRADE DISTRIBUTION")
    print("=" * 100)
...
def print_final_decision(aggregate_df: pd.DataFrame):
    print()
    print("FINAL DIRECTIONAL CONTEXT V3.1 DECISION")
    print("=" * 100)
    ...
    print("- V3.1 es deliberadamente conservador.")
    print("- Si mejora mucho el drawdown pero queda con pocos trades, sirve como filtro defensivo.")
    print("- Si el par robusto se mantiene positivo, se conserva para futuras estrategias.")
    print("- No habilita paper trading; solo valida contexto direccional calibrado.")
...
def main():
    ...
    print("VALIDATE DIRECTIONAL CONTEXT FILTER V3.1")
    print("=" * 100)
    print("Purpose: conservative calibrated context filter based on Fase 1.23")
    print()
    ...
    results_output = reports_dir / "directional_context_v3_1_window_results.csv"
    aggregate_output = reports_dir / "directional_context_v3_1_aggregate.csv"
    trades_output = reports_dir / "directional_context_v3_1_trade_diagnostics.csv"
    ...
    print()
    print("DIRECTIONAL CONTEXT V3.1 AGGREGATE RESULTS")
    print("=" * 100)
    print(
        aggregate_df[
            [
    ...
    print()
    print("DIRECTIONAL CONTEXT V3.1 DETAIL BY WINDOW")
    print("=" * 100)
    print(
        results_df[
            [
    ...
    print_base_vs_v3_1(results_df)
    print_v3_1_context_distribution(trades_all_df)

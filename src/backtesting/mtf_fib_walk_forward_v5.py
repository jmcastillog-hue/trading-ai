...
from backtesting.mtf_fib_backtester_v5_tp1_be import (
...
def build_candidate_configs():
    ...
    for min_drop in min_drop_values:
...
def run_config_on_period(
    ...
    report = run_mtf_fib_short_backtest_v5_tp1_be(
        htf_df=htf_df,
        ltf_df=ltf_df,
        ...
        tp1_close_weight=config["tp1_close_weight"],
...
def config_to_text(
    ...
    return (
        ...
        f"close={config['tp1_close_weight']}"
    )
...
def choose_best_config(
    ...
    for index, config in enumerate(configs, start=1):
        ...
        print(
            f"Probando config {index}/{len(configs)} | "
            f"drop={config['min_drop_pct']} | "
            f"tp1={config['tp1_ratio']}",
            flush=True
        )
...
def run_walk_forward_validation_v5(
    htf_df,
    ltf_df,
    train_months=8,
    test_months=2,
    ...
    print(
        f"\nConfiguraciones V5 candidatas: {len(configs)}\n"
    )
    ...
    for window in windows:
        ...
        summary_rows.append({
            ...
            "tp1_ratio": best_config["tp1_ratio"],
            "tp1_close_weight": best_config["tp1_close_weight"],
